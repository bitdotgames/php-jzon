...
inline void next(const char** input)
{
  ++*input;
}

inline char current(const char** input)
{
  return **input;
}
...
char* parse_string_internal(const char** input, JzonAllocator* allocator)
{
  ...
  while (current(input))
  {
    if (current(input) == '\\' && *((*input) + 1) && *((*input) + 1) == '"')
    {
      next(input);
      next(input);
    }
...
const char* parse_string(const char** input, JzonValue* output, JzonAllocator* allocator)
{
...
const char* parse_array(const char** input, JzonValue* output, JzonAllocator* allocator)
{ 
  if (current(input) != '[')
    return "Expected '['";
  ...
  while (current(input))
  {
    ...
    const char* error = parse_value(input, value, allocator);
...
const char* parse_object(const char** input, JzonValue* output, bool root_object, JzonAllocator* allocator)
{
  if (current(input) == '{')
    next(input);
  else if (!root_object)
...
const char* parse_number(const char** input, JzonValue* output)
{
  bool is_float = false;
  char* start = (char*)*input;
  ...
  if (is_float)
  {
    ...
    output->float_value = strtod(start, NULL);
  }
  else
  {
    output->is_int = true;
    output->int_value = strtol(start, NULL, 10);
  }

  return NULL;
}

const char* parse_true(const char** input, JzonValue* output)
{
...
const char* parse_false(const char** input, JzonValue* output)
{
...
const char* parse_null(const char** input, JzonValue* output)
{
...
const char* parse_value(const char** input, JzonValue* output, JzonAllocator* allocator)
{
  ...
  //zend_error(E_NOTICE, "SWITCH: %c", ch);
  ...
  switch (ch)
  {
    ...
    default: return ch >= '0' && ch <= '9' ? parse_number(input, output) : "Unexpected symbol";
  }
}
...
JzonParseResult jzon_parse_custom_allocator(const char* input, JzonAllocator* allocator)
{
  ...
  if(current(&input) && current(&input) == '[')
    error = parse_array(&input, output, allocator);
  else
    error = parse_object(&input, output, true, allocator);
  if(error == NULL)
  {
    skip_whitespace(&input);
    if(current(&input))
  ...
  result.error = error;
  result.error_pos = error != NULL ? (input - orig) : 0;
  return result;
}
